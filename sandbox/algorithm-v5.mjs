...
//   BORDERLINE (临界健康)   → 高曝光
//   WEAK (微弱)            → 中曝光
//   SUSPICIOUS (救命期)     → 高曝光 + 警告标 (需要更多人来救/判, 30 天观察)
//   CRITICAL (病危)        → 中-低曝光 + 警告标 (趋势继续向死, 曝光渐降)
//   HEARTBEAT (心跳, 病危末期) → 5% 曝光 (最后救命机会, 强心剂可回 SUSPICIOUS)
//   DEAD (死亡)            → 0 曝光, 不可复活, 用户须新建
// ======================================================================
export const MARKER_STATUS_V4 = {
  HEALTHY: 'healthy', BORDERLINE: 'borderline', WEAK: 'weak',
  SUSPICIOUS: 'suspicious',     // 救命期
  CRITICAL: 'critical',         // 病危
  HEARTBEAT: 'heartbeat',       // 病危末期心跳
  SUNK: 'sunk',                 // 死亡 (不可复活)
  ARCHIVED: 'archived',
};

// v5: 判定 — 是否处于"救命期" (有征兆但还能救)
//   触发任一即进救命期:
//     A. 急转 — 历史正向但近期负向
//     B. 量级反转 — 累积偏负但量级大 (那棵树/雪崩警告类)
//     C. 持续衰退 — 多窗口 sentiment 持续下行
//     D. brigade 疑似 — 历史正向 + 近期突然集中 reports
//     E. 死寂过期 — 累积过 10 但近期长时间无新互动
function isInSuspiciousPhase(marker, now) {
  const t = trendAnalysis(marker, now);
  const calendarDays = daysBetween(marker.tCreate, now);
  const cumLikes = (marker.likes || []).length;
  const cumReports = (marker.reports || []).length;
  const total = cumLikes + cumReports;

  // 强正向 mark 不进救命期 — ratio > 0.85
  if (total >= 5 && cumLikes / total >= 0.85) return false;

  // A. 急转
  if (t.overall.sentiment >= 0.15 && t.recent.sampleSize >= 3 &&
      t.recent.sentiment <= -0.1) return true;

  // B. 死寂过期
  if (total >= 10 && calendarDays > 60) {
    const recent60 = sentimentInWindow(marker, now, 60);
    if (recent60.sampleSize === 0) return true;
  }
  ...
  if (w30.sampleSize >= 3 && w90Older.sampleSize >= 3 &&
      w90Older.sentiment > 0.15 && w30.sentiment < w90Older.sentiment - 0.3) return true;

  // D. 近期举报为主
  const recent90 = sentimentInWindow(marker, now, 90);
  if (recent90.sampleSize >= 5 && recent90.posCount < recent90.negCount &&
      recent90.sentiment <= -0.2 && t.overall.sentiment >= 0.1) return true;

  // E. 量级反转
  if (total >= 30 && cumLikes / total < 0.5 && cumLikes >= 5) return true;

  // F. brigade 疑似
  if (t.overall.sentiment >= 0.5 && total >= 20) {
    const recent14 = sentimentInWindow(marker, now, 14);
    if (recent14.negCount >= 5 && recent14.posCount === 0) return true;
  }

  // G. 持续争议 — 累积量足够 (>=10) 且比例接近 1:1 (0.4-0.65)
  if (total >= 10) {
    const ratio = cumLikes / total;
    if (ratio >= 0.40 && ratio <= 0.65) return true;
  }

  // H. 渐进恶化 — 累积比 < 0.55 + 近期更负
  if (total >= 30 && cumLikes / total < 0.55) {
    if (t.recent.sampleSize >= 3 && t.recent.sentiment < 0) return true;
  }

  return false;
}

// v5: 判定 — 是否处于"病危期" (救命期内继续恶化)
//   病危条件:
//     - 救命期内近期 sentiment 持续向死 (新低)
//     - 累积负向比例较高 (ratio < 0.4)
//     - 长期低互动 + 近期负
function isInCriticalPhase(marker, now) {
  const t = trendAnalysis(marker, now);
  const cumLikes = (marker.likes || []).length;
  const cumReports = (marker.reports || []).length;
  const total = cumLikes + cumReports;
  ...
  // 累积比例严重偏负 (< 0.40) — 直接进病危 (放宽)
  if (ratio < 0.40 && total >= 8) return true;

  // 累积比例偏负 (< 0.45) + 近期 sentiment 也负
  if (ratio < 0.45 && t.recent.sampleSize >= 3 && t.recent.sentiment < -0.3) return true;

  // 量级大 + 累积负
  if (total >= 30 && ratio < 0.40 && t.overall.sentiment < -0.2) return true;

  // 持续多窗口都负
  if (t.overall.sentiment < -0.3 && t.recent.sampleSize >= 5 && t.recent.sentiment < -0.3) return true;
...
// v5: 判定 — 是否处于"心跳期" (病危末期)
//   心跳条件:
//     - 病危状态持续, 累积比例严重偏负 (< 0.25)
//     - 长期累积负向且近期仍无好转
function isInHeartbeatPhase(marker, now) {
  const cumLikes = (marker.likes || []).length;
  const cumReports = (marker.reports || []).length;
  const total = cumLikes + cumReports;
  ...
  // 累积严重偏负 + 量级足够
  if (ratio < 0.25 && total >= 8) return true;

  // 大量级 + 极偏负
  if (total >= 20 && ratio < 0.30) {
    const recent60 = sentimentInWindow(marker, now, 60);
    if (recent60.sampleSize >= 3 && recent60.sentiment < -0.4) return true;
  }
...
function isDead(marker, now) {
  ...
  // 极严重负向且量级足够 — 真死
  if (ratio < 0.15 && total >= 10) {
    // 检查是否有强心剂 (近 30 天有 likes)
    const recent30 = sentimentInWindow(marker, now, 30);
    if (recent30.posCount === 0) return true;
  }

  // 量级巨大 + 极偏负 (sub-0.20)
  if (total >= 30 && ratio < 0.20) {
    const recent30 = sentimentInWindow(marker, now, 30);
    if (recent30.posCount <= 1) return true;
  }
...
export function markerStatusV4(marker, now, reporterStats = {}) {
  ...
  // v5 状态机: 优先判死亡, 然后心跳/病危/救命期, 最后健康程度
  if (isDead(marker, now)) return MARKER_STATUS_V4.SUNK;

  // 心跳: 病危末期 (5% 曝光)
  if (isInHeartbeatPhase(marker, now)) return MARKER_STATUS_V4.HEARTBEAT;

  // 病危: 趋势继续向死, 曝光渐降
  if (isInCriticalPhase(marker, now)) return MARKER_STATUS_V4.CRITICAL;

  // 救命期: 有征兆, 仍展示但带警告
  if (isInSuspiciousPhase(marker, now)) return MARKER_STATUS_V4.SUSPICIOUS;
  ...
  // 兜底: 极低曝光但没触发心跳/病危 — 给 weak (信号不足保留)
  return MARKER_STATUS_V4.WEAK;
}
...
export function shouldRenderV4(marker, now, rng = Math.random, reporterStats = {}) {
  ...
  if (s === MARKER_STATUS_V4.SUNK) return false;        // 死亡
  if (s === MARKER_STATUS_V4.HEARTBEAT) return rng() < 0.05;  // 心跳 5% 曝光
  if (s === MARKER_STATUS_V4.CRITICAL) return rng() < 0.30;   // 病危 30% 曝光
  // SUSPICIOUS 仍 100% 展示 (要数据来救/判)
  // healthy/borderline/weak 全展示
  return true;
}
