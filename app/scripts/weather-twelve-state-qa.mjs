...
const outputDir = path.resolve('..', 'docs', 'qa', 'visual-north-star', 'weather-full-frame-integration-gate');
...
await board('weather-12-state-board-v2.jpg', allPanels, { columns: 4 });
await board('same-world-geometry-review-v2.jpg', allPanels, { columns: 4, guides: true });

for (const time of timeStates) {
  await board(`${time}-row-comparison-v2.jpg`, weatherStates.map(weather => ({
    file: `${weather.file}-${time}-home-390x844.png`,
    label: `${weather.label} ${time.toUpperCase()}`,
  })), { columns: 4 });
}
...
await board('night-sky-restraint-review.jpg', [
  { file: 'sunny-night-home-390x844.png', label: 'SUNNY · RESTRAINED STARS' },
  { file: 'cloudy-night-home-390x844.png', label: 'CLOUDY · STARS OBSCURED' },
  { file: 'rainy-night-home-390x844.png', label: 'RAINY · NO STARS' },
  { file: 'snowy-night-home-390x844.png', label: 'SNOWY · CLEAR WINTER STARS' },
], { columns: 4 });
