...
export function initMapbox() {
  if (Platform.OS === 'web') {
    ...
    // Do not start broad offline-pack downloads at app/auth launch. On a
    // fresh install those requests competed with the first visible style and
    // delayed the exact Hike/Memory tiles the user was waiting for. Each map
    // now owns truthful style readiness and an app-native loading/error state.
  }
}
