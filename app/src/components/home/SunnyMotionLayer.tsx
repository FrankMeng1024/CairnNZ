import React, { useEffect, useMemo, useRef, useState } from 'react';
import {
  AccessibilityInfo,
  Animated,
  AppState,
  type AppStateStatus,
  Platform,
  StyleSheet,
  View,
} from 'react-native';
import { useIsFocused } from '@react-navigation/native';
import * as Battery from 'expo-battery';

const CLOUD_LAYER = require('../../../assets/home/motion/sunny/cloud-motion-layer.webp');
const WATER_LAYER = require('../../../assets/home/motion/sunny/water-motion-layer.webp');
const VEGETATION_LAYER = require('../../../assets/home/motion/sunny/vegetation-motion-layer.webp');

function isStaticQaOverride(): boolean {
  if (!__DEV__ || Platform.OS !== 'web') return false;

  try {
    const search = (globalThis as typeof globalThis & {
      location?: { search?: string };
    }).location?.search;
    return new URLSearchParams(search ?? '').get('homeMotion') === 'off';
  } catch {
    return false;
  }
}

function timing(value: Animated.Value, toValue: number, duration: number) {
  return Animated.timing(value, {
    toValue,
    duration,
    useNativeDriver: true,
    isInteraction: false,
  });
}

class StaticFallbackBoundary extends React.Component<
  React.PropsWithChildren,
  { failed: boolean }
> {
  state = { failed: false };

  static getDerivedStateFromError() {
    return { failed: true };
  }

  render() {
    return this.state.failed ? null : this.props.children;
  }
}

/**
 * Gate A1 Sunny ambient life.
 *
 * The approved static image remains canonical. These three sparse alpha
 * overlays repeat pixels from that exact image, so initialization failure,
 * Reduce Motion, low-power mode, or lifecycle suspension always resolves to
 * the complete approved still rather than a partial scene.
 */
function SunnyMotionLayerImpl() {
  const isFocused = useIsFocused();
  const qaStatic = useMemo(isStaticQaOverride, []);
  const [appState, setAppState] = useState<AppStateStatus>(AppState.currentState);
  const [reduceMotion, setReduceMotion] = useState<boolean | null>(null);
  const [lowPowerMode, setLowPowerMode] = useState<boolean | null>(null);

  const cloudX = useRef(new Animated.Value(-1.1)).current;
  const cloudY = useRef(new Animated.Value(0.05)).current;
  const cloudOpacity = useRef(new Animated.Value(0.082)).current;
  const waterX = useRef(new Animated.Value(-0.65)).current;
  const waterY = useRef(new Animated.Value(0)).current;
  const waterOpacity = useRef(new Animated.Value(0.1)).current;
  const vegetationX = useRef(new Animated.Value(0)).current;
  const vegetationY = useRef(new Animated.Value(0)).current;
  const vegetationOpacity = useRef(new Animated.Value(0.044)).current;

  useEffect(() => {
    const appStateSubscription = AppState.addEventListener('change', setAppState);
    const reduceMotionSubscription = AccessibilityInfo.addEventListener(
      'reduceMotionChanged',
      setReduceMotion,
    );
    let lowPowerSubscription: { remove: () => void } | null = null;

    AccessibilityInfo.isReduceMotionEnabled()
      .then(setReduceMotion)
      .catch(() => setReduceMotion(true));
    if (Platform.OS === 'web') {
      // Expo Battery intentionally has no web event emitter. The native
      // iOS/Android path below remains the production Low Power Mode gate.
      setLowPowerMode(false);
    } else {
      Battery.isLowPowerModeEnabledAsync()
        .then(setLowPowerMode)
        .catch(() => setLowPowerMode(true));
      try {
        lowPowerSubscription = Battery.addLowPowerModeListener(({ lowPowerMode: enabled }) => {
          setLowPowerMode(enabled);
        });
      } catch {
        // A constrained/older native runtime should fail safe to the static
        // canonical scene rather than running unmonitored ambient work.
        setLowPowerMode(true);
      }
    }

    return () => {
      appStateSubscription.remove();
      reduceMotionSubscription.remove();
      lowPowerSubscription?.remove();
    };
  }, []);

  const shouldAnimate =
    isFocused &&
    appState === 'active' &&
    reduceMotion === false &&
    lowPowerMode === false &&
    !qaStatic;

  useEffect(() => {
    if (!shouldAnimate) return;

    cloudX.setValue(-1.1);
    cloudY.setValue(0.05);
    cloudOpacity.setValue(0.082);
    waterX.setValue(-0.65);
    waterY.setValue(0);
    waterOpacity.setValue(0.1);
    vegetationX.setValue(0);
    vegetationY.setValue(0);
    vegetationOpacity.setValue(0.044);

    const cloud = Animated.loop(
      Animated.parallel([
        Animated.sequence([
          timing(cloudX, 1, 9870),
          timing(cloudX, 3.1, 11130),
          timing(cloudX, 1, 11130),
          timing(cloudX, -1.1, 9870),
        ]),
        Animated.sequence([
          timing(cloudY, -0.08, 9870),
          timing(cloudY, 0.04, 11130),
          timing(cloudY, -0.08, 11130),
          timing(cloudY, 0.05, 9870),
        ]),
        Animated.sequence([
          timing(cloudOpacity, 0.098, 9870),
          timing(cloudOpacity, 0.088, 11130),
          timing(cloudOpacity, 0.098, 11130),
          timing(cloudOpacity, 0.082, 9870),
        ]),
      ]),
    );

    const water = Animated.loop(
      Animated.parallel([
        Animated.sequence([
          timing(waterX, 0.55, 1270),
          timing(waterX, 1.05, 2210),
          timing(waterX, -0.28, 1740),
          timing(waterX, -0.65, 1480),
        ]),
        Animated.sequence([
          timing(waterY, 0.08, 1270),
          timing(waterY, -0.06, 2210),
          timing(waterY, 0.06, 1740),
          timing(waterY, 0, 1480),
        ]),
        Animated.sequence([
          timing(waterOpacity, 0.185, 1270),
          timing(waterOpacity, 0.135, 2210),
          timing(waterOpacity, 0.19, 1740),
          timing(waterOpacity, 0.1, 1480),
        ]),
      ]),
    );

    const vegetation = Animated.loop(
      Animated.parallel([
        Animated.sequence([
          timing(vegetationX, 0.22, 3940),
          timing(vegetationX, 0.04, 1020),
          Animated.delay(3680),
          timing(vegetationX, -0.24, 1020),
          timing(vegetationX, 0, 3040),
        ]),
        Animated.sequence([
          timing(vegetationY, -0.06, 3940),
          timing(vegetationY, 0, 1020),
          Animated.delay(3680),
          timing(vegetationY, 0.04, 1020),
          timing(vegetationY, 0, 3040),
        ]),
        Animated.sequence([
          timing(vegetationOpacity, 0.054, 3940),
          timing(vegetationOpacity, 0.047, 1020),
          Animated.delay(3680),
          timing(vegetationOpacity, 0.056, 1020),
          timing(vegetationOpacity, 0.044, 3040),
        ]),
      ]),
    );

    cloud.start();
    water.start();
    vegetation.start();

    return () => {
      cloud.stop();
      water.stop();
      vegetation.stop();
    };
  }, [
    cloudOpacity,
    cloudX,
    cloudY,
    shouldAnimate,
    vegetationOpacity,
    vegetationX,
    vegetationY,
    waterOpacity,
    waterX,
    waterY,
  ]);

  if (!shouldAnimate) return null;

  return (
    <View
      pointerEvents="none"
      accessibilityElementsHidden
      importantForAccessibility="no-hide-descendants"
      style={StyleSheet.absoluteFill}
      testID="sunny-motion-layer"
    >
      <Animated.Image
        source={CLOUD_LAYER}
        resizeMode="stretch"
        style={[
          styles.cloudLayer,
          {
            opacity: cloudOpacity,
            transform: [
              { translateX: cloudX },
              { translateY: cloudY },
              { scale: 1.003 },
            ],
          },
        ]}
        testID="sunny-motion-cloud"
      />
      <Animated.Image
        source={WATER_LAYER}
        resizeMode="stretch"
        style={[
          styles.waterLayer,
          {
            opacity: waterOpacity,
            transform: [
              { translateX: waterX },
              { translateY: waterY },
              { scale: 1.0015 },
            ],
          },
        ]}
        testID="sunny-motion-water"
      />
      <Animated.Image
        source={VEGETATION_LAYER}
        resizeMode="stretch"
        style={[
          styles.vegetationLayer,
          {
            opacity: vegetationOpacity,
            transform: [
              { translateX: vegetationX },
              { translateY: vegetationY },
            ],
          },
        ]}
        testID="sunny-motion-vegetation"
      />
    </View>
  );
}

export function SunnyMotionLayer() {
  return (
    <StaticFallbackBoundary>
      <SunnyMotionLayerImpl />
    </StaticFallbackBoundary>
  );
}

const styles = StyleSheet.create({
  // Coordinates are the exact alpha bounds from the approved 426×924 QA
  // overlays, mapped into the fixed 375×812 production canvas. Cropping
  // transparent pixels lowers decoded overlay memory and compositor area
  // without moving or reauthoring any visible content.
  cloudLayer: {
    position: 'absolute',
    left: 0,
    top: 7.03,
    width: 375,
    height: 374.52,
  },
  waterLayer: {
    position: 'absolute',
    left: 0,
    top: 363.87,
    width: 271.13,
    height: 152.1,
  },
  vegetationLayer: {
    position: 'absolute',
    left: 241.2,
    top: 226.75,
    width: 133.8,
    height: 271.52,
  },
});
