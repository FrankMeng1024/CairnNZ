...
// Summaries are small and track geometry remains stored separately/on demand,
// so retaining 500 synced summaries supports a 300-Activity archive without
// loading hundreds of full traces into the list.
...
interface SessionState {
  ...
  deleteSession: (id: string) => Promise<{ remoteState: 'deleted' | 'queued' }>;
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  deleteSession: async (id) => {
    ...
    const remoteSessionId = session?.remoteId
      ?? session?.serverActivityId
      ?? (session && /^\d+$/.test(session.id) ? Number(session.id) : null);
    ...
    if (session && clientActivityId && get().currentUserId === userId) {
      ...
      crashLogger.breadcrumb(`session:delete:remote ok=${ok} target=${remoteSessionId ?? clientActivityId}`);
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
    ...
    // A retained Activity tombstone is the durable retry contract. The UI can
    // truthfully distinguish an acknowledged server delete from a queued one
    // without resurrecting the locally removed Activity.
    return { remoteState };
  },
