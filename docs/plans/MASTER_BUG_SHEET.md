...
| V2-CONFLICT-3 | A9 PERSISTED state semantics | **OVERRIDDEN 2026-06-11 by user product decision: Plant button requires A1=LOCKED + 0.5s stable in ALL cases. Cold start with persisted arOrigin still waits 1-3s for ARKit to settle. Rationale: industry AR products (Pokemon GO, Niantic Wayfarer, Snap Lens) all gate placement on AR-ready, not on persisted-state. The previous "PERSISTED → instant enable" UX was technically faster but produced cairn-mis-placement during AR-init.** Original (now superseded): PERSISTED = arOriginLocked=true (button enabled). GPS_LOCKED = arOriginLocked=true (button enabled). |
