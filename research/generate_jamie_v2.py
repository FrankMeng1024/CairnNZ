"""根据 Jamie 改后的文字，重画 4 帧 SVG。
帧 1: 周六一早进山（朝阳，开始独行的喜悦）
帧 2: 来到一个新地方（陌生山脊，夕阳，孤独的味道）
帧 3: 看见前人温暖的留言（夕阳，AR 石堆 + 暖色气泡）
帧 4: 看海录温暖的话（开阔海岸，暖色 mark）
"""
import json

JAMIE_ART = []

# 帧 1: 朝阳, 路在脚下, 出发的样子 (基调: 清晨蓝-粉)
JAMIE_ART.append('''<svg viewBox="0 0 320 240" preserveAspectRatio="xMidYMid meet" class="pain-svg">
<defs>
...
# 帧 3: 蹲下, 看见前人温暖的留言, AR 石堆 + 暖色气泡 + 落日
JAMIE_ART.append('''<svg viewBox="0 0 320 240" preserveAspectRatio="xMidYMid meet" class="pain-svg">
<defs>
...
# 输出 — 替换 view-overview.js 里 jamie 部分
out_lines = []
for s in JAMIE_ART:
...
with open('C:/ClaudeCodeProjects/Cairn/research/jamie_art_new.js','w',encoding='utf-8') as f:
    f.write(out)
print('OK')
