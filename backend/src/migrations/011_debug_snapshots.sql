-- Migration 011: Debug snapshots for AR rendering verification
-- Purpose: receive PNG screenshots from app's debug 🐛 button so we can
--   visually verify what user actually sees on screen (e.g. white-strand
--   bug debugging in v133-v141 — telemetry breadcrumbs alone insufficient).
...
CREATE TABLE IF NOT EXISTS debug_snapshots (
  ...
  -- Diagnostic metadata (free-form JSON: marker count, tracking state,
  -- materials registered, ARKit origin, etc.)
  meta JSON,
  ...
  -- Source info
  ar_mode VARCHAR(16),              -- 'sphere' | 'ritual'
