...
function createMemoryPool() {
  ...
  async function execute(sql, params = []) {
    ...
    if (compact.startsWith('SELECT id, created_geometry_hash')) {
      const found = state.routes.find(row => row.user_id === params[0] && row.client_route_id === params[1]);
      return [[found].filter(Boolean)];
    }
    if (compact.startsWith('SELECT id, permission, audience_epoch FROM routes')) {
      const found = state.routes.find(row => row.id === params[0] && row.user_id === params[1]);
      return [[found].filter(Boolean)];
    }
    if (compact.startsWith('SELECT id FROM routes WHERE user_id')) {
      const found = state.routes.find(row => row.user_id === params[0] && row.client_route_id === params[1]);
      return [[found ? { id: found.id } : undefined].filter(Boolean)];
    }
    if (compact.startsWith('INSERT IGNORE INTO route_audience_epochs')
      || compact.startsWith('UPDATE route_audience_epochs SET ends_at')
      || compact.startsWith('DELETE FROM shared_route_leases')
      || compact.startsWith('DELETE FROM route_audience_epochs')) {
    ...
    if (compact.startsWith('INSERT INTO routes')) {
      ...
      const row = {
        ...
        audience_epoch: 1, run_count: 0, last_run_at: null,
    ...
    if (compact.startsWith('UPDATE routes SET name = ?')) {
      ...
      if (params[8] === 1) row.audience_epoch += 1;
      return [{ affectedRows: 1 }];
    }
